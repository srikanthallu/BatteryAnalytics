...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 17 08:51:14 2022
...
class M2Dataset(Dataset):
    def __init__(self, images_dir, label_dir, idk_dir, embed_dir, local_HW, transform=None, 
                 imhw=(224,256)):
        ...
        for image_id in fids:
            ...
            idk_arr=np.load(idk_dir+image_id[:-4]+'._idk.npy')
        ...
        for i in range(self.im_sz):
            ...
            for j in range(self.im_sz):
                ...
                if y<0 or y>=self.orig_im_sz[1]:
                    continue
                local_image[i,j]=image[x,y]
        
        images=np.array([local_image])
        images=torch.as_tensor(images).float()#.contiguous()
        local_image=Image.fromarray(local_image, 'L')
        
        if self.transform is not None:
            images = self.transform(local_image)
...
def main():
    ...
    dir_idk='../../output/output-idk(ffn)/train/'
    dir_embed = '../../output/output-idk(ffn)/train/'
    train_dataset=M2Dataset(dir_img,dir_label,dir_idk,5)
    
    train_loader = DataLoader(train_dataset, batch_size=256, 
                              shuffle= False,
                              num_workers=8, pin_memory=True, 
                              sampler=None)
    for idx, (images, labels, img_id, pos2d) in enumerate(train_loader):
