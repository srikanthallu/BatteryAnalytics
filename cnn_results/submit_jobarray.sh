...
python cnn_battery_analytics.py --window $WINDOW --horizon $HORIZON --n-filters $N_FILTERS --kernel-size $KERNEL_SIZE --hidden-dim $HIDDEN_DIM --transfer $TRANSFER --n-epochs $N_EPOCHS --n-split $N_SPLIT --training-filter $TRAINING_FILTER --model-id $MODEL_ID --features $FEATURES --infile $FILENAME
				...
				for N_FILTERS in "${N_FILTERS_LIST[@]}"; do
					for KERNEL_SIZE in "${KERNEL_SIZE_LIST[@]}"; do
						for HIDDEN_DIM in "${HIDDEN_DIM_LIST[@]}"; do
							for TRANSFER in "${TRANSFER_LIST[@]}"; do
