...
from batteryanalytics.nn import ANN
...
def train_model(samples, n_split, features, target, window, horizon, hidden_dim, transfer, n_epochs, training_filter, model_id):
	model = ANN(
		window=window,
		horizon=horizon,
	...
	filename = os.path.join(
		ba_utils.get_ann_dirname([key for key,value in samples_items[test_idx]]),
		ba_utils.get_ann_basename(features, model.window, model.horizon, model.hidden_dim, model.transfer, model.n_epochs, training_filter, model_id)
	).replace("joblib", "npz")
...
def main(infile, n_split, features, window, horizon, hidden_dim, transfer, n_epochs, training_filter, model_id):
	...
	train_model(samples, n_split, features, target, window, horizon, hidden_dim, transfer, n_epochs, training_filter, model_id)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		type=int,
		required=True,
		help="ANN hidden dimension size(s)"
	)
