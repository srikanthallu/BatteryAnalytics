...
python ann_battery_analytics.py --window $WINDOW --horizon $HORIZON --hidden-dim $HIDDEN_DIM --transfer $TRANSFER --n-epochs $N_EPOCHS --n-split $N_SPLIT --training-filter $TRAINING_FILTER --model-id $MODEL_ID --features $FEATURES --infile $FILENAME
